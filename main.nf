...
process ASCATSC {
    tag "Copy number calling using ASCAT.sc"
    labels 'process_high'
    ...
    script:
        """
        Rscript ASCATsc_run.R --bamdir ${bams} --genome ${genome} --binsizes ${binsize} \\
        --sex ${sex} --chroms ${chroms} --penalty ${penalty} --multipcf ${multipcf} \\
        --threads ${task.cpus} 
        """
...
workflow {
    ...
    // CNA Calling
    //ASCATSC(deduped.bam.collect(), 
            //params.genome, params.sex, 
            //params.chroms, params.binsize, 
            //params.penalty, params.multipcf)
